...
def get_git_revision():
    ...
    try:
        try:
            # A build made exactly at a release tag IS that release: fork
            # release tags (e.g. v4.26.0-coral302) must name it, or release
            # bundle filenames won't match the tag being released.
            tag = _git("describe", "--exact-match", "--match", "v[0-9]*",
                       "--exclude", "*-golden")
        except subprocess.CalledProcessError:
            # Anchored describe for dev builds. Marker tags (v*-golden),
            # fork release tags (v*-coral*), and forwarded upstream-* tags
            # must not leak into descendants' describe output.
            tag = _git("describe", "--match", "v[0-9]*",
                       "--exclude", "*-golden", "--exclude", "*-coral*")
        if _is_dirty():
            tag += "-dirty"
    except subprocess.CalledProcessError:
        tag = "v9.9.9-dev"
