...
//! Full-width sliver pinned to the very top of the screen: idle states on the
//! left and projected battery life right-aligned on the SAME row whenever the
//! two fit side by side; the battery only wraps onto a second row when they
//! would collide. 18 or 34 px instead of the old 84 px box.
...
static void prv_update_proc(Layer *layer, GContext *ctx) {
  ...
  if (charge_state.is_charging) {
    ...
    snprintf(batt_line, sizeof(batt_line), "%" PRIu32 "d %" PRIu32 "h %" PRIu32 "m",
  ...
  const bool one_row = (idle_size.w + HUD_TEXT_GAP + batt_size.w) <= w;

  const int16_t hud_height = (one_row ? HUD_LINE_HEIGHT : 2 * HUD_LINE_HEIGHT) + 2;
  const GRect box = GRect(0, 0, bounds.size.w, hud_height);
  graphics_context_set_fill_color(ctx, GColorBlack);
  graphics_fill_rect(ctx, &box);

  graphics_context_set_text_color(ctx, GColorWhite);
  // Gothic 14 renders with ~2 px of leading; shift up so the rows fit the sliver.
  ...
  graphics_draw_text(ctx, batt_line, font, one_row ? line1 : line2, GTextOverflowModeFill,
