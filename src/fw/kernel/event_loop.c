...
void launcher_main_loop(void) {
  ...
  while (1) {
    ...
    // While idle-waiting, the KernelBG liveness timer feeds our watchdog bit
    // for us (see system_task_idle_timer_callback), so both tasks check in on
    // ONE shared wake instead of two drifting ones. The long timeout here is
    // only a failsafe if that timer ever stops.
