...
static bool prv_reversed_start(GAPLEConnection *connection, uint16_t conn_handle,
  ...
  // Reversed clients are keyed by conn_handle, not by phone slot; use the
  // invalid-slot sentinel so slot-based invalidation leaves them alone (they're
  // torn down via unsubscribe/disconnect and ppogatt_invalidate_all_references).
  PPoGATTClient *client = prv_create_client(rx_ack_timer, send_retry_timer, PHONE_SLOT_INVALID);
  if (!client) {
    return false;
  }
...
void ppogatt_handle_service_discovered(BLECharacteristic *characteristics, PhoneSlot slot) {
  ...
  bt_lock();
  {
    ...
    // Create new clients:
    PPoGATTClient *client = prv_create_client(rx_ack_timer, send_retry_timer, slot);
    if (!client) {
      bt_unlock();
      new_timer_delete(rx_ack_timer);
      new_timer_delete(send_retry_timer);
