...
static void system_task_idle_timer_callback(void* data) {
  ...
  // wake carries both liveness check-ins instead of two drifting timers. A
  // busy KernelMain feeds itself at the top of its loop; a wedged one feeds
  // nobody and the watchdog fires as before.
